...
// Update createTables function to include money table
func CreateTables(db *sql.DB) error {
	tables := []struct {
		...
		{"money", createMoneyTable}, // Add money table
	}
	...
	// Update existing loto table to add checks column
	if err := addLotoChecksColumn(db); err != nil {
		log.Printf("Warning: Could not add checks column to loto table: %v", err)
	}
...
// Add money table creation function
func createMoneyTable(db *sql.DB) error {
	const stmt = `
	...
	// Initialize with one record if table is empty
	const initStmt = `
	INSERT OR IGNORE INTO money (id, sum) 
	SELECT 1, 0 
	WHERE NOT EXISTS (SELECT 1 FROM money WHERE id = 1);
	`
	_, err := db.Exec(initStmt)
	return err
}

// Add checks column to existing loto table
func addLotoChecksColumn(db *sql.DB) error {
	// Check if column already exists
	const checkStmt = `
	SELECT COUNT(*) as count 
	FROM pragma_table_info('loto') 
	WHERE name = 'checks';
	`
	var count int
	...
	if count == 0 {
		const addColumnStmt = `ALTER TABLE loto ADD COLUMN checks BOOLEAN DEFAULT FALSE;`
...
// Add these updates to your createIndexes function in schema.go
...
func createIndexes(db *sql.DB) error {
	indexes := []string{
		...
		// Add new index for loto checks
		"CREATE INDEX IF NOT EXISTS idx_loto_checks ON loto(checks)",
		...
		// Add indexes for money table
		"CREATE INDEX IF NOT EXISTS idx_money_sum ON money(sum)",
		"CREATE INDEX IF NOT EXISTS idx_money_updated_at ON money(updated_at)",
	}
